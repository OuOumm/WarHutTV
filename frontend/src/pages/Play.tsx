...
const Play = () => {
  ...
  const loadSources = async (title?: string) => {
    ...
    try {
      const response = await apiClient.get('/search', { params: { wd: title || detail?.vod_name || '' } });
      const data = response.data;
      ...
      if (typeof data === 'object') {
        Object.entries(data).forEach(([key, val]: [string, any]) => {
          if (val?.list?.length > 0) {
            ...
            sourceList.push({ key, name: item.source_name || key, poster: item.vod_pic, episodeCount: val.list.length, speed: null, status: 'done' });
          }
        });
      }
  ...
  const testAllSources = useCallback(async () => {
    if (sources.length === 0) return;
    setTestingAll(true);
    setSources(prev => prev.map(s => ({ ...s, status: 'testing' as const, speed: null })));

    const testOne = async (source: SourceItem, index: number) => {
      try {
        const searchRes = searchDataCache
          ? { data: { [source.key]: searchDataCache[source.key] } }
        ...
        if (siteData?.list?.length > 0) {
          ...
          if (epUrl && epUrl.includes('.m3u8')) {
            ...
            setSources(prev => { const next = [...prev]; next[index] = { ...next[index], speed: result, status: 'done' }; return next; });
            return;
          }
        }
        setSources(prev => { const next = [...prev]; next[index] = { ...next[index], status: 'done' }; return next; });
      } catch {
        setSources(prev => { const next = [...prev]; next[index] = { ...next[index], status: 'error' }; return next; });
      }
    };

    await Promise.allSettled(sources.map((s, i) => testOne(s, i)));
    setTestingAll(false);
  }, [sources, detail]);
