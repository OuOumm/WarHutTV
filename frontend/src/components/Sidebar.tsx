...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen transition-all duration-300 border-r border-glass-border z-10 shadow-lg bg-card backdrop-blur-xl ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      <div className="flex h-full flex-col">
        ...
        <div className="relative h-16 flex items-center justify-center">
          {!collapsed && (
            <span className="text-2xl font-bold text-primary tracking-tight">WarHutTV</span>
          )}
          <button
            onClick={() => setCollapsed(!collapsed)}
            className={`absolute top-1/2 -translate-y-1/2 flex items-center justify-center w-8 h-8 rounded-lg text-muted hover:text-text hover:bg-surface transition-colors ${
        ...
        <nav className="px-2 mt-4 space-y-1">
          {navItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
              className={`group flex items-center rounded-lg px-2 py-2 pl-4 text-muted hover:bg-primary-glow hover:text-primary font-medium transition-colors duration-200 min-h-[40px] gap-3 ${
                isActive(item.path) ? 'bg-primary-glow text-primary' : ''
              }`}
            >
              <div className="w-4 h-4 flex items-center justify-center">
                <svg className={`h-4 w-4 ${isActive(item.path) ? 'text-primary' : 'text-muted group-hover:text-primary'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="space-y-1">
            {menuItems.map((item) => (
              <NavLink
                key={item.path}
                to={item.path}
                className={`group flex items-center rounded-lg px-2 py-2 pl-4 text-sm text-muted hover:bg-primary-glow hover:text-primary transition-colors duration-200 min-h-[40px] gap-3 ${
                  isActive(item.path) ? 'bg-primary-glow text-primary' : ''
                }`}
              >
                <div className="w-4 h-4 flex items-center justify-center">
